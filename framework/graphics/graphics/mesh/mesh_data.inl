...
{
    template<typename T>
	void MeshData::addData(const std::string name, const DataInfo &info, const T &value)
	{
	...
	template <typename T>
	T MeshData::getData(const std::string name) const
	{
	...
	template<typename T>
	void MeshData::setData(const std::string name, const T &value)
	{
		std::vector<Byte> temp(sizeof(T));
		memcpy(temp.data(), &value, sizeof(T));
	...
	template<typename T>
	void MeshData::addData(const std::string name, const DataInfo &info, const std::vector<T> &values)
	{
	...
	template <typename T>
	std::vector<T> MeshData::getData(const std::string name, const uint32_t count) const
	{
		const auto& bytes = getValue(name, mapDatas);
	...
	template<typename T>
	void MeshData::setData(const std::string name, const std::vector<T> &values)
	{
		std::vector<Byte> temp(sizeof(T) * values.size());
		memcpy(temp.data(), values.data(), temp.size());
		setValue(name, temp, mapDatas);
	...
	template<typename T>
	void MeshData::addData(const std::string name, const DataInfo &info, const T * const pSrc, const uint32_t count)
	{
	...
	template<typename T>
	void MeshData::getData(const std::string name, const T * const pDst, const uint32_t count)
	{
		const auto& bytes = getValue(name, mapDatas);
	...
	template<typename T>
	void MeshData::setData(const std::string name, const T * const pSrc, const uint32_t count)
	{
		std::vector<Byte> temp(sizeof(T) * count);
		if (count != 0u && pSrc != nullptr) memcpy(temp.data(), pSrc, temp.size());
		setValue(name, temp, mapDatas);
