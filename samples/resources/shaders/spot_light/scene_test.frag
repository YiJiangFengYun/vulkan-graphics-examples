...
layout (location = 4) in vec3 inLightVec[MAX_LIGHT_COUNT];
layout (location = 6) in vec4 inShadowCoord[MAX_LIGHT_COUNT];
...
{   
    ...
    //     // outFragColor.rgb = vec3(texture(shadowMaps[i], inUV).r, 0.0, 0.0);
    // }
